...
import {
    Search, ChevronDown, ShoppingBag, Package, Download, Filter, CheckCircle2, Clock, Trash2, RotateCcw
} from 'lucide-react';
...
export default function Purchases() {
    ...
    const fetchPurchases = useCallback(async (silent = false) => {
        ...
        const [purchasesRes, returnsRes] = await Promise.all([
            supabase.from('purchases').select(`*, products(name)`).order('date', { ascending: false }),
            supabase.from('supplier_returns').select('invoice_number')
        ]);
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-brand-charcoal tracking-tight flex items-center gap-3">
                        <div className="w-12 h-12 bg-brand-red rounded-2xl flex items-center justify-center shadow-red-200 shadow-lg"><ShoppingBag className="text-white" size={24} /></div>
                        Purchase Dashboard
                    </h1>
                    <p className="text-slate-700 mt-1 font-medium">Manage stock inwards and supplier invoices</p>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 relative">
                <div className="relative group flex-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-brand-red transition-colors" size={18} /><input type="text" placeholder="Search invoices or suppliers..." className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:border-brand-red outline-none transition-all shadow-sm placeholder:text-slate-400" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>

                <div className="relative">
                    <button
                        onClick={() => setIsFilterOpen(!isFilterOpen)}
                        className={`px-5 py-3 bg-white border border-slate-200 rounded-2xl text-slate-800 hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2 font-bold text-sm ${isFilterOpen ? 'border-brand-red text-brand-red' : ''}`}
                    >
                    ...
                    {isFilterOpen && (
                        <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 z-[100] animate-slide-up">
                            <div className="space-y-4">
                                <div>
                                    <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2">Order Status</h4>
                                    <div className="space-y-2">
                                        {['pending', 'received', 'returned'].map(s => (
                                            <label key={s} className="flex items-center gap-2 cursor-pointer group">
                                                ...
                                                />
                                                <span className="text-xs font-bold text-slate-600 capitalize group-hover:text-brand-red transition-colors">{s}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                <div className="pt-4 border-t border-slate-50">
                                    <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2">Payment Status</h4>
                                    <div className="space-y-2">
                                        {['unpaid', 'partial', 'paid'].map(s => (
                                            <label key={s} className="flex items-center gap-2 cursor-pointer group">
                                                <input
                                                    ...
                                                    className="w-4 h-4 rounded border-slate-300 text-brand-red focus:ring-brand-red cursor-pointer"
                                                />
                                                <span className="text-xs font-bold text-slate-600 capitalize group-hover:text-brand-red transition-colors">{s === 'paid' ? 'Full' : s}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                <div className="pt-4 border-t border-slate-50">
                                    <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2">Purchase Type</h4>
                                    <div className="space-y-2">
                                        {[
                                            ...
                                            <label key={t.id} className="flex items-center gap-2 cursor-pointer group">
                                                <input
                                                    ...
                                                    className="w-4 h-4 rounded border-slate-300 text-brand-red focus:ring-brand-red cursor-pointer"
                                                />
                                                <span className="text-xs font-bold text-slate-600 capitalize group-hover:text-brand-red transition-colors">{t.label}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                <button
                                    ...
                                    className="w-full py-2 text-[10px] font-black text-slate-400 hover:text-brand-red uppercase tracking-widest text-center transition-colors"
                                >
                                    Reset Filters
                                </button>
                            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-shrink-0">
                    <div className="sticky top-6 space-y-4">
                        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4 w-full lg:w-72">
                            <div className="space-y-4">
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                            <h3 className="text-[10px] font-black text-slate-600 border-b border-slate-50 pb-3 mb-4 uppercase tracking-[0.2em]">Summary</h3>
                            <div className="space-y-4">
                                <div><p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Total Cost</p><p className="text-xl font-black text-brand-charcoal font-data">₱{filteredPurchases.reduce((sum, g) => sum + g.grand_total, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div>
                                ...
                                <div><p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Complete Orders</p><p className="text-xl font-black text-brand-charcoal font-data">{filteredPurchases.length}</p></div>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-slate-50/50 border-b border-slate-100">
                                        <th className="px-6 py-4 text-[11px] font-black text-slate-600 uppercase tracking-widest pl-16">Created</th>
                                        <th className="px-6 py-4 text-[11px] font-black text-slate-600 uppercase tracking-widest">Invoice / Supplier</th>
                                        <th className="px-6 py-4 text-[11px] font-black text-slate-600 uppercase tracking-widest">Status / Payment</th>
                                        <th className="px-6 py-4 text-[11px] font-black text-slate-600 uppercase tracking-widest">Received Date</th>
                                        <th className="px-6 py-4 text-[11px] font-black text-brand-charcoal uppercase tracking-widest text-right">Total Amount</th>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {loading ? (
                                            ...
                                            return (
                                                <Fragment key={group.invoice_number || group.date}>
                                                    <tr
                                                        ...
                                                        className={`hover:bg-slate-50/80 transition-all cursor-pointer group ${expandedInvoices.has(group.invoice_number) ? 'bg-slate-50/50' : ''}`}
                                                    >
                                                        <td className="px-6 py-5">
                                                            <div className="flex items-center gap-3">
                                                                <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-brand-red/10 group-hover:text-brand-red transition-colors">
                                                                    <Clock size={16} />
                                                                </div>
                                                                <div>
                                                                    <p className="text-[10px] font-black text-brand-charcoal uppercase tracking-wider">{format(creationDate, 'MMM d, yyyy')}</p>
                                                                    <p className="text-[9px] font-bold text-slate-400 uppercase">{format(creationDate, 'p')}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-5">
                                                            <div className="flex items-center gap-4">
                                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${group.status === 'received' ? 'bg-emerald-50 text-emerald-500' : 'bg-orange-50 text-orange-500'}`}>
                                                                    {group.status === 'received' ? <CheckCircle2 size={18} /> : <Clock size={18} />}
                                                                </div>
                                                                <div>
                                                                    <div className="flex items-center gap-2">
                                                                        <p className="text-sm font-black text-brand-charcoal uppercase font-data">{group.invoice_number || 'N/A'}</p>
                                                                    ...
                                                                    <p className="text-[10px] font-bold text-slate-400 uppercase">{group.supplier}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-5">
                                                            <div className="flex flex-col gap-1">
                                                                {group.status === 'pending' ? (
                                                                    <button
                                                                        ...
                                                                        className="w-fit group/btn flex items-center gap-1.5 text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border bg-orange-50 text-orange-600 border-orange-100 hover:bg-emerald-600 hover:text-white hover:border-emerald-700 transition-all shadow-sm"
                                                                    >
                                                                    ...
                                                                    <span className="w-fit flex items-center gap-1.5 text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border bg-emerald-50 text-emerald-600 border-emerald-100">
                                                                        <CheckCircle2 size={8} />
                                                                        <span>{group.status}</span>
                                                                    </span>
                                                                )}
                                                                {group.payment_status === 'paid' ? (
                                                                    <span className="w-fit text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border bg-emerald-50 text-emerald-600 border-emerald-100 flex items-center gap-1">
                                                                        <CheckCircle2 size={8} />
                                                                        PAID
                                                                    </span>
                                                                ) : (
                                                                ...
                                                                {group.purchase_type === 'transfer' && (
                                                                    <span className="w-fit text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border bg-blue-50 text-blue-600 border-blue-100">TRANSFER</span>
                                                                )}
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-5">
                                                            {receiptDate ? (
                                                                <div className="flex flex-col">
                                                                    <p className="text-[10px] font-black text-brand-charcoal uppercase tracking-wider">{format(receiptDate, 'MMM d, yyyy')}</p>
                                                                    <p className="text-[9px] font-bold text-slate-400 uppercase">{format(receiptDate, 'p')}</p>
                                                                </div>
                                                            ) : (
                                                        ...
                                                        <td className="px-6 py-5 text-right flex items-center justify-end gap-2">
                                                            ...
                                                            <div className={`p-2 rounded-lg transition-all ${expandedInvoices.has(group.invoice_number) ? 'bg-brand-red text-white rotate-180' : 'bg-slate-50 text-slate-300'}`}><ChevronDown size={16} /></div>
                                                        </td>
                                                    </tr>
                                                                ...
                                                                <div className="p-6 border-l-4 border-indigo-500 bg-white shadow-inner rounded-2xl space-y-4">
                                                                    <div className="flex items-center justify-between border-b pb-2">
                                                                        ...
                                                                        <div className="flex gap-2">
                                                                            {group.status === 'pending' && <button onClick={(e) => { e.stopPropagation(); if (confirm('Mark this order as Received? This will update live inventory levels.')) handleMarkAsReceived(group); }} className="text-[9px] font-black bg-emerald-500 text-white px-3 py-1 rounded uppercase tracking-tighter">MARK AS RECEIVED</button>}
                                                                    ...
                                                                    <div className="space-y-3">
                                                                        {group.items.map((item, idx) => (
                                                                            <div key={idx} className="flex justify-between items-start text-xs border-b border-slate-50 pb-2 last:border-0">
                                                                                <div>
                                                                                    <p className="font-bold text-brand-charcoal">{item.products?.name}</p>
                                                                                    <p className="text-[9px] text-slate-400 font-data">UNIT PRICE: ₱{Number(item.unit_price).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                                                                                </div>
                                                                                <div className="text-right">
                                                                                    <p className="font-bold text-brand-charcoal">x{item.quantity}</p>
                                                                                    <p className="text-[10px] text-slate-500 font-data font-black">₱{item.total_price.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                                                                                </div>
                                                                            </div>
                                                                        ))}
                                                                    </div>
                                                                    <div className="pt-2 border-t flex flex-col items-end gap-1">{group.total_discount > 0 && <span className="text-[10px] text-brand-orange">Discount: -₱{group.total_discount.toLocaleString()}</span>}<span className="text-sm font-black text-brand-red font-data">Total: ₱{group.grand_total.toLocaleString()}</span></div>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    )}
                                        ...
                                        <tr><td colSpan={6} className="py-20 text-center text-slate-400">No purchases found for this filters.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
