...
import { encodePrice, isEncoded, decodePrice } from '../lib/priceCodes';
...
export default function AdminPricelist() {
    ...
    return (
        <div className="space-y-6 animate-fade-in max-w-none">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-sm sticky top-0 z-40 backdrop-blur-md bg-white/90">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-brand-red rounded-2xl flex items-center justify-center shadow-red">
                        <Tag className="text-white" size={24} />
                    </div>
                    <div>
                        <h1 className="text-xl font-black text-brand-charcoal tracking-tight font-data">ADMIN PRICELIST</h1>
                        <div className="flex items-center gap-3 mt-1">
                            <div className="flex flex-col">
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none mb-1">Total SRP Value</p>
                                <p className="text-sm font-black text-brand-charcoal font-data leading-none">₱{stats.totalSrpValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                            </div>
                            <div className="w-px h-6 bg-slate-200" />
                            <div className="flex flex-col">
                                <div className="flex items-center gap-2 mb-1">
                                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">Total Cost Value</p>
                                    <button
                                        onClick={() => setShowWsp(!showWsp)}
                                        className={`flex items-center gap-1 text-[8px] font-black uppercase tracking-tighter transition-all px-1.5 py-0.5 rounded ${showWsp ? 'text-brand-red bg-red-50' : 'text-slate-400 hover:text-brand-red border border-slate-200'}`}
                                    >
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 focus-within:border-brand-red focus-within:ring-2 focus-within:ring-brand-red/10 transition-all w-80">
                        <Search size={14} className="text-slate-400 flex-shrink-0" />
                        <input
                            type="text"
                            placeholder="Search items or brands..."
                            className="bg-transparent border-none outline-none text-xs w-full placeholder:text-slate-400 text-slate-900 font-bold"
                            value={searchTerm}
            ...
            <div className="flex gap-6 items-start">
                ...
                <div className="hidden xl:block w-56 sticky top-24 space-y-1 bg-white p-3 rounded-2xl border border-slate-200 shadow-sm overflow-y-auto max-h-[calc(100vh-140px)] custom-scrollbar">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-2 mb-3">Master Groups</p>
                    <button
                        onClick={() => setCategoryFilter('All')}
                        className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all ${categoryFilter === 'All' ? 'bg-brand-red text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                        All Categories
                    </button>
                    {allCategories.map(cat => (
                        <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all truncate ${categoryFilter === cat ? 'bg-brand-red text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
                            {cat}
                        </button>
                    ))}
                ...
                <div className="flex-1 space-y-8 pb-10">
                    <div className="xl:hidden flex items-center gap-3">
                        <div className="relative flex-1">
                            ...
                            <select
                                className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-xs font-black text-brand-charcoal appearance-none shadow-sm outline-none focus:ring-2 focus:ring-brand-red/10"
                    ...
                    {loading ? (
                        ...
                        sortedL1s.map(l1 => {
                            ...
                            return (
                                <div key={l1} className="space-y-3">
                                    <div
                                        onClick={() => toggleL1(l1)}
                                        className="flex items-center justify-between border-l-4 border-brand-charcoal pl-4 py-2 bg-slate-100/30 rounded-r-xl cursor-pointer group hover:bg-slate-100/50 transition-all font-data"
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`p-1 rounded-md transition-all ${isL1Expanded ? 'bg-brand-charcoal text-white' : 'bg-white text-slate-400 border border-slate-200'}`}>
                                                {isL1Expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                                            </div>
                                            <h2 className="text-lg font-black text-brand-charcoal tracking-tighter uppercase">{l1} PRICELIST</h2>
                                        </div>
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest pr-4">
                                    ...
                                    <div className={`grid grid-cols-1 gap-6 transition-all duration-300 ${isL1Expanded ? 'max-h-[10000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                                        {Object.keys(grouped[l1]).sort().map(subCat => {
                                            ...
                                            return (
                                                <div key={subCat} className="bento-card overflow-hidden ml-4 md:ml-8 border-slate-200/60 shadow-sm relative">
                                                    <div
                                                        onClick={() => toggleSubCat(subCatKey)}
                                                        className="px-6 py-2.5 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between cursor-pointer hover:bg-slate-100/50 transition-colors"
                                                    >
                                                        <h3 className="text-[10px] font-black text-slate-600 uppercase tracking-widest flex items-center gap-2">
                                                            <div className={`p-0.5 rounded transition-all ${isExpanded ? 'bg-brand-charcoal text-white' : 'bg-slate-200 text-slate-500'}`}>
                                                                {isExpanded ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
                                                            </div>
                                                            {subCat}
                                                        </h3>
                                                        <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter">{grouped[l1][subCat].length} items</span>
                                                    </div>
                                                        ...
                                                        <table className="min-w-full">
                                                            <thead>
                                                                <tr className="bg-white border-b border-slate-100">
                                                                    <th className="px-6 py-2 text-left text-[9px] font-black uppercase tracking-widest text-slate-400">Spec / Description</th>
                                                                    <th className="px-6 py-2 text-center text-[9px] font-black uppercase tracking-widest text-slate-400 w-24">Brand</th>
                                                                    <th className="px-6 py-2 text-right text-[9px] font-black uppercase tracking-widest text-slate-600 bg-slate-50/50 w-32">SRP (₱) Selling</th>
                                                                    <th className="px-6 py-2 text-right text-[9px] font-black uppercase tracking-widest text-brand-red bg-red-50/20 w-44">WSP (Cost)</th>
                                                                    <th className="px-6 py-2 text-center text-[9px] font-black uppercase tracking-widest text-slate-400 w-28 text-orange-600">Low Stock Trigger</th>
                                                                </tr>
                                                            </thead>
                                                                ...
                                                                {grouped[l1][subCat].map((p) => (
                                                                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                                                                        <td className="px-6 py-3">
                                                                            <p className="text-[11px] font-black text-brand-charcoal tracking-tight leading-tight uppercase">
                                                                                {p.name.split(' > ').slice(-1)[0]}
                                                                            </p>
                                                                        </td>
                                                                        <td className="px-6 py-3 text-center">
                                                                            {p.brand && (
                                                                                <span className="inline-flex px-1.5 py-0.5 rounded bg-slate-100 text-[8px] font-black text-slate-500 uppercase tracking-widest">
                                                                                    {p.brand}
                                                                                </span>
                                                                            )}
                                                                        </td>
                                                                        <td className="px-6 py-3 text-right bg-slate-50/20">
                                                                            {editingId?.id === p.id && editingId?.field === 'srp' ? (
                                                                                <div className="flex items-center justify-end gap-1">
                                                                                    <input
                                                                                        autoFocus
                                                                                        className="w-24 bg-white border-2 border-slate-900 rounded-lg px-2 py-1 text-xs font-black text-right outline-none font-data"
                                                                                        value={editValue}
                                                                                    ...
                                                                                    />
                                                                                    <button onClick={() => handleSavePrice(p.id, 'srp')} className="p-1 bg-brand-charcoal text-white rounded"><Save size={12} /></button>
                                                                                </div>
                                                                            ) : (
                                                                                <div
                                                                                    onClick={() => { setEditingId({ id: p.id, field: 'srp' }); setEditValue((p.selling_price ?? 0).toString()); }}
                                                                                    className="group/price flex items-center justify-end gap-2 cursor-pointer hover:text-brand-charcoal transition-colors"
                                                                                >
                                                                                    <span className="text-[11px] font-black text-slate-600 font-data tracking-tight">
                                                                                        ₱{(p.selling_price ?? 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
                                                                                    </span>
                                                                                    <Edit size={10} className="text-slate-300 opacity-0 group-hover/price:opacity-100" />
                                                                                </div>
                                                                            )}
                                                                        </td>
                                                                        <td className="px-6 py-3 text-right bg-red-50/10">
                                                                            {editingId?.id === p.id && editingId?.field === 'wsp' ? (
                                                                                <div className="flex items-center justify-end gap-1">
                                                                                    <input
                                                                                        autoFocus
                                                                                        className="w-24 bg-white border-2 border-brand-red rounded-lg px-2 py-1 text-xs font-black text-right outline-none font-data"
                                                                            ...
                                                                            {editingId?.id === p.id && editingId?.field === 'trigger' ? (
                                                                                <div className="flex items-center justify-center gap-1">
                                                                                    <input
                                                                                        autoFocus
                                                                                        type="number"
                                                                                        className="w-16 bg-white border-2 border-orange-500 rounded-lg px-2 py-1 text-xs font-black text-center outline-none font-data"
                                                                                        value={editValue}
                                                                                        onChange={(e) => setEditValue(e.target.value)}
                                                                                        onKeyDown={(e) => {
                                                                                ...
                                                                                <div
                                                                                    ...
                                                                                    className="group/trigger flex items-center justify-center gap-2 cursor-pointer hover:bg-orange-50/50 rounded-lg py-1 transition-colors"
                                                                                >
                                                                                    <span className="text-[11px] font-black text-orange-600 font-data">
                                                                                        {p.low_stock_threshold || 10}
                                                                                    </span>
                                                                                    <Edit size={10} className="text-orange-200 opacity-0 group-hover/trigger:opacity-100" />
                                                                                </div>
                                                                            )}
                                                                        </td>
                                                                    </tr>
                        ...
                        <div className="bento-card py-20 text-center">
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <p className="text-[11px] text-slate-400 font-black uppercase tracking-widest">No items found</p>
                            </div>
                        </div>
                    )}
                </div>
