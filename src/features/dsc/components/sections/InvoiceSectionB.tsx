...
export function InvoiceSectionB({ dayId, branchId, readOnly = false }: Props) {
  const { data: invoices = [], isLoading } = useDscInvoices(dayId, 'B');
  ...
  const save = () => {
    batchUpsert.mutate({ dayId, series: 'B', items: rows.filter((r) => r.description.trim()) });
    setDirty(false);
  };
  ...
  return (
    <div className="bento-card p-2.5 space-y-2.5 overflow-hidden">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-black text-text-primary uppercase tracking-wider">Invoice B</h3>
      ...
      <div className="flex justify-end text-xs font-black text-text-secondary pt-1.5 border-t border-border-default no-print">
        <div className="flex items-baseline gap-2">
          <span className="uppercase tracking-widest text-[9px] text-text-muted">Total B:</span>
