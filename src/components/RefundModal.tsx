...
export default function RefundModal({ isOpen, onClose, onSuccess }: RefundModalProps) {
    ...
    const fetchProducts = async () => {
        const { data, error } = await supabase.from('products').select('id, sku, name, selling_price').order('name');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { data, error: insertError } = await supabase.from('customer_refunds').insert(refundRecords).select('*, products(name, sku)');
                                ...
                                {items.map((item, index) => (
                                    <div key={index} className="flex gap-4 p-4 border rounded-2xl bg-white hover:border-brand-red/30 transition-all">
                                        <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                                            <div className="md:col-span-2"><select required className="modal-input" value={item.productId} onChange={e => updateItem(index, { productId: e.target.value })}><option value="">Select product...</option>{products.map(p => <option key={p.id} value={p.id}>{p.sku} — {p.name}</option>)}</select></div>
